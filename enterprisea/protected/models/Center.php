...
class Center extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('type_id,center_name,name,m_name,l_name,surname,dob,birth_place,gender,age,nationality,email,pp_country,pp_province,pp_city,pp_suburb,pp_address1,pp_zipcode', 'required'),
	...
	public function check($attribute,$params)
    {
		...
		if((Yii::app()->controller->action->id=='update') and $this->$attribute!='')
		{
			$user = User::model()->findByAttributes(array('id'=>$this->uid));
			//var_dump($user);exit; 
			if($user->email != $this->$attribute)
			{
				$validate = User::model()->findByAttributes(array('email'=>$this->$attribute));
				if($validate!=NULL)
				{
	...
	public function attributeLabels()
	{
		return array(
			...
			'tenant' => 'Tenant',
			'type_id' => 'Type',
			'center_name' => 'Support Center Name',
			'pd_title' => 'Pd Title',
			'name' => 'First Name',
			'm_name' => 'Second Name',
			'l_name' => 'Third Name',
