<script>
	...
	function addRow1(tableID) 
	{
		...
		if(rowCount < 16)// limit the user from creating fields more than your limits
		{
  ...
  <?php $documents = PrivateTutorUploads::model()->findAllByAttributes(array('center_id'=>$_REQUEST['id'])); ?>
    <?php
			...
			?>
				...
				<?php echo CHtml::link('Edit', array('privateTutorUploads/update','id'=>$document->center_id,'document_id'=>$document->id),array('class'=>''));  ?></span>
                <span class="btn btn-danger">
				<i class="glyphicon glyphicon-trash"></i>
				<?php echo CHtml::link('Delete', array('privateTutorUploads/deletes','id'=>$document->center_id,'document_id'=>$document->id),array('class'=>'','confirm'=>'Are you sure?'));  ?></span>
                </div>
                </div>
    <?php }}else{ ?>
		
		<div class="panel-body document_list">
    	<div class="col-sm-8"><h5 class="subtitle">No document(s) uploaded</h5></div>
        </div>
...
<div class="panel-footer">
          ...
          <?php echo CHtml::Link('Continue ',array('privateTutor/view','id'=>$_REQUEST['id']),array('class'=>'btn btn-orange')); ?>  
