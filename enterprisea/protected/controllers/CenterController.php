...
class CenterController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view', 'captcha', 'details'),
				'users'=>array('*'),
			),
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update','loadcategories','createcourse','ajaxcreate','ajaxedit','ajaxupdate','ajaxdelete','loadbatches','loadsubjects','complete','viewcourse'
							,'loadprovinces','loadcities','loadsuburbs'),
				'users'=>array('@'),
			),
	...
	public function actionCreate()
	{
		...
		$model=new Center;
		$model->location_lat = -34.397;
		$model->location_long = 150.644;
		$model_1 = new CenterExperience;
		$model_course= new CenterCourse;
		...
		if(!empty($_POST))
		{
			...
			$model->location_lat = $_POST['Center']['location_lat'];
			$model->location_long = $_POST['Center']['location_long'];
			...
			if(isset($_POST['CenterCourse']))
			{	
				foreach($_POST['CenterCourse']['course_id'] as $val)
				{	
					$model_course= new CenterCourse;
			...
			else
			{
				...
				$model_1->attributes=$_POST['CenterExperience'];
			...
			if($validate)
			{
				if($model->save())
				{
					...
					$uid = Center::model()->add_admin($tenant,$model->id);//add admin user for center
					$model->saveAttributes(array('uid'=>$uid));
					Center::model()->add_settings($tenant);//add default settings to db
					Center::model()->add_weekdays($tenant);//add default weekdays to db
					Center::model()->add_grading_levels($tenant);//add default Grade levels to db
	...
	public function actionUpdate($id)
	{
		...
		$model_1 = CenterExperience::model()->findByAttributes(array('center_id'=>$model->id));
		$model_course= new CenterCourse;
		$model_service = new ServiceCenter;
		...
		if(!empty($_POST))
		{
			...
			if(isset($_POST['CenterCourse']))
			{	
				$old_courses = CenterCourse::model()->findAllByAttributes(array('center_id'=>$model->id));
				...
				CenterCourse::model()->deleteAllByAttributes(array('center_id'=>$model->id));
				foreach($_POST['CenterCourse']['course_id'] as $val)
				{	
					$model_course= new CenterCourse;
					$model_course->center_id = $model->id;
	...
	public function actionDelete($id)
	{
		...
		CenterCourse::model()->deleteAllByAttributes(array('center_id'=>$id));
		
		//delete experience
		CenterExperience::model()->deleteAllByAttributes(array('center_id'=>$id));
		//delete location details
		CenterLocation::model()->deleteAllByAttributes(array('center_id'=>$id));
		
		//delete service details
		ServiceCenter::model()->deleteAllByAttributes(array('center_id'=>$id));
		
		//delete uploaded documents
		$documents = CenterUploads::model()->findAllByAttributes(array('center_id'=>$id));
		foreach($documents as $document)
		{
			
			$destination_file = 'uploadedfiles/center_document/'.$document->center_id.'/'.$document->file;
		...
		//delete center
		$center_user = Center::model()->findByPk($id);
		User::model()->deleteAllByAttributes(array('id'=>$center_user->uid));
		Center::model()->deleteAllByAttributes(array('id'=>$id));
	...
	public function actionIndex()
	{
		...
		$centers= Center::model()->findAllByAttributes(array('user_type'=>1));
		...
		if($subjects)
		{
			if($batch->delete())
			{
				...
				if($reuslt)
				{
					...
					$criteria->condition	= '`t`.`category`=:category';  //AND `acp`.`category_id` IS NULL
					$criteria->params		= array(':category'=>$_POST['category_id']);
					$grades	= CHtml::listData(Course::model()->findAll($criteria), 'id', 'name' );
					$arr['content'] = "<option value=''>Select Batch</option>";
	...
	{
	   $data=Provinces::model()->findAll('country_id=:x', 
	   array(':x'=>$_POST['Country']));
	...
	{
	   $data=Cities::model()->findAll('province_id=:x', 
	   array(':x'=>$_POST['Province']));
	...
	{
	   $data=Suburbs::model()->findAll('city_id=:x', 
	   array(':x'=>$_POST['City']));
