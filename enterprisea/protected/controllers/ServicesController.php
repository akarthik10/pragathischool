...
class ServicesController extends RController
{
	/**
	...
	public function actionAjaxcreate()
	{
		...
		if(isset($_POST['Services']))
		{
			...
			if($model->validate()){
				if($model->save()){
					echo json_encode(array('status'=>'success', 'message'=>'Product & Services Added'));
					Yii::app()->end();
				}
			}
			else{
	...
	public function actionDelete($id)
	{
		if(Yii::app()->request->isPostRequest)
		{
			...
			ServiceCenter::model()->deleteAllByAttributes(array('service_id'=>$id));
	...
	public function actionIndex()
	{
		...
		$total = Services::model()->count($criteria);
		$pages = new CPagination($total);
        $pages->setPageSize(20);
        $pages->applyLimit($criteria);  // the trick is here!
		$posts = Services::model()->findAll($criteria);
