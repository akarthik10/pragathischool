...
class SharedTutorUploadsController extends Controller
{
	/**
...
*/
	...
	{
		$model = SharedTutorUploads::model()->findByAttributes(array('center_id'=>$id));
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['SharedTutorUploads']))
		{
			
			$list = $_POST['SharedTutorUploads'];
			...
			{	$value = explode('.',$_FILES['SharedTutorUploads']['name']['file'][$i]);
				$model=new SharedTutorUploads;
				$model->center_id = $_POST['SharedTutorUploads']['center_id'][$i];
				$model->title = $_POST['SharedTutorUploads']['title'][$i];
				...
				$model->file_type = $_FILES['SharedTutorUploads']['type']['file'][$i];
				
				$file_size = $_FILES['SharedTutorUploads']['size']['file'][$i];
				if($model->center_id!='' and $model->title!='' and $model->file!='' and $model->file_type!='') // Checking if Document name and file is uploaded
				{
					if(in_array($model->file_type,$valid_file_types)) // Checking file type
					{
						if($file_size <= 5242880) // Checking file size
						{
							...
							if(!is_dir('uploadedfiles/sharedtutor_document/')) // Creating student_document directory
							{
								mkdir('uploadedfiles/sharedtutor_document/');
							}
							if(!is_dir('uploadedfiles/sharedtutor_document/'.$model->center_id)) // Creating student directory for saving the files
							{
								mkdir('uploadedfiles/sharedtutor_document/'.$model->center_id);
							}
							$temp_file_loc = $_FILES['SharedTutorUploads']['tmp_name']['file'][$i];
							$destination_file = 'uploadedfiles/sharedtutor_document/'.$model->center_id.'/'.$model->file;
			...
			if($flag == 1) // If no errors, go to next step of the student registration
			{
				...
				if($_POST['SharedTutorUploads']['document']==1)
				{
					$this->redirect(array('sharedTutorUploads/create','id'=>$_POST['SharedTutorUploads']['sid']));
				}
				else
				{
				
					$this->redirect(array('sharedTutorUploads/view','id'=>$_POST['SharedTutorUploads']['sid']));
				}
			}
			else // If errors are present, redirect to the same page
			{
				...
				{
					$this->redirect(array('create','id'=>$_POST['SharedTutorUploads']['sid']));
				}
			}
		} // END isset
	...
	public function actionUpdate()
	{
		...
		if(isset($_POST['SharedTutorUploads']))
		{
			...
			{
				if($_FILES['SharedTutorUploads']['name']['file']!=NULL)
				{
					$value = explode('.',$_FILES['SharedTutorUploads']['name']['file']);
					$extension = end($value); // Get extension of the file
					$model->file = $this->generateRandomString(rand(6,10)).'.'.$extension; // Generate random string as filename
					$model->file_type = $_FILES['SharedTutorUploads']['type']['file'];
					$file_size = $_FILES['SharedTutorUploads']['size']['file'];
					if(in_array($model->file_type,$valid_file_types)) // Checking file type
					{
						if($file_size <= 5242880) // Checking file size
						{
							...
							$temp_file_loc = $_FILES['SharedTutorUploads']['tmp_name']['file'];
							$destination_file = 'uploadedfiles/sharedtutor_document/'.$model->center_id.'/'.$model->file;
			...
			{ 
				if($model->save())
				{
					if($_FILES['SharedTutorUploads']['name']['file']!=NULL)
					{
						$old_destination_file = 'uploadedfiles/sharedtutor_document/'.$model->center_id.'/'.$old_model['file'];	
						if(file_exists($old_destination_file))
						{
							unlink($old_destination_file);
						}
					}
					$this->redirect(array('sharedTutorUploads/create','id'=>$model->center_id));
				}
				else
				{
	...
	public function actionDeletes()
	{
		...
		$this->redirect(array('sharedTutorUploads/create','id'=>$_REQUEST['id']));
	}
	...
	{
		$dataProvider=new CActiveDataProvider('SharedTutorUploads');
	...
	*/
	public function actionDownload()
	{
		...
		$file_path = 'uploadedfiles/sharedtutor_document/'.$model->center_id.'/'.$model->file;
	...
	*/
	...
	{
		$model = SharedTutorUploads::model()->findByAttributes(array('id'=>$_REQUEST['id']));
	...
	*/
	...
	{
		$model=new SharedTutorUploads('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['SharedTutorUploads']))
			$model->attributes=$_GET['SharedTutorUploads'];
	...
	{
		$model=SharedTutorUploads::model()->findByPk($id);
		if($model===null)
