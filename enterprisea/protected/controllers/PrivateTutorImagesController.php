...
class PrivateTutorImagesController extends Controller
{
	/**
...
*/
	...
	{
		$model = PrivateTutorImages::model()->findByAttributes(array('center_id'=>$id));
	...
	{
		//echo $_POST['PrivateTutorImages']['sid'];exit;
		$model=new PrivateTutorImages;
		$flag = 1;
		$valid_file_types = array('image/jpeg','image/png'); // Creating the array of valid file types
		$files_not_saved = '';
		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['PrivateTutorImages']))
		{
			
			$list = $_POST['PrivateTutorImages'];
			...
			{	$value = explode('.',$_FILES['PrivateTutorImages']['name']['file'][$i]);
				$model=new PrivateTutorImages;
				$model->center_id = $_POST['PrivateTutorImages']['center_id'][$i];
				$model->title = $_POST['PrivateTutorImages']['title'][$i];
				...
				$model->file_type = $_FILES['PrivateTutorImages']['type']['file'][$i];
				
				$file_size = $_FILES['PrivateTutorImages']['size']['file'][$i];
				if($model->center_id!='' and $model->title!='' and $model->file!='' and $model->file_type!='') // Checking if Document name and file is uploaded
				{
					if(in_array($model->file_type,$valid_file_types)) // Checking file type
					{
						if($file_size <= 5242880) // Checking file size
						{
							...
							if(!is_dir('uploadedfiles/privatetutor_images/')) // Creating student_document directory
							{
								mkdir('uploadedfiles/privatetutor_images/');
							}
							if(!is_dir('uploadedfiles/privatetutor_images/'.$model->center_id)) // Creating student directory for saving the files
							{
								mkdir('uploadedfiles/privatetutor_images/'.$model->center_id);
							}
							$temp_file_loc = $_FILES['PrivateTutorImages']['tmp_name']['file'][$i];
							$destination_file = 'uploadedfiles/privatetutor_images/'.$model->center_id.'/'.$model->file;
					...
					{
						$flag = 0;
						Yii::app()->user->setFlash('errorMessage',"Only files with these extensions are allowed: jpg, png.");
					}
				}
				elseif($model->title=='' and $model->file_type!='') // If document name is empty
				{
					$flag = 0;
					Yii::app()->user->setFlash('errorMessage',"Image Name cannot be empty!");
			...
			if($flag == 1) // If no errors, go to next step of the student registration
			{
				...
				if($_POST['PrivateTutorImages']['document']==1)
				{
					$this->redirect(array('privateTutorImages/create','id'=>$_POST['PrivateTutorImages']['sid']));
				}
				else
				{
				
					$this->redirect(array('privateTutorImages/view','id'=>$_POST['PrivateTutorImages']['sid']));
				}
			}
			else // If errors are present, redirect to the same page
			{
				...
				{
					$this->redirect(array('create','id'=>$_POST['PrivateTutorImages']['sid']));
				}
			}
		} // END isset
	...
	public function actionUpdate()
	{
		...
		if(isset($_POST['PrivateTutorImages']))
		{
			...
			{
				if($_FILES['PrivateTutorImages']['name']['file']!=NULL)
				{
					$value = explode('.',$_FILES['PrivateTutorImages']['name']['file']);
					$extension = end($value); // Get extension of the file
					$model->file = $this->generateRandomString(rand(6,10)).'.'.$extension; // Generate random string as filename
					$model->file_type = $_FILES['PrivateTutorImages']['type']['file'];
					$file_size = $_FILES['PrivateTutorImages']['size']['file'];
					if(in_array($model->file_type,$valid_file_types)) // Checking file type
					{
						if($file_size <= 5242880) // Checking file size
						{
							...
							$temp_file_loc = $_FILES['PrivateTutorImages']['tmp_name']['file'];
							$destination_file = 'uploadedfiles/privatetutor_images/'.$model->center_id.'/'.$model->file;
			...
			{ 
				if($model->save())
				{
					if($_FILES['PrivateTutorImages']['name']['file']!=NULL)
					{
						$old_destination_file = 'uploadedfiles/privatetutor_images/'.$model->center_id.'/'.$old_model['file'];	
						if(file_exists($old_destination_file))
						{
							unlink($old_destination_file);
						}
					}
					$this->redirect(array('privateTutorImages/create','id'=>$model->center_id));
				}
				else
				{
	...
	public function actionDeletes()
	{
		...
		$this->redirect(array('privateTutorImages/create','id'=>$_REQUEST['id']));
	}
	...
	{
		$dataProvider=new CActiveDataProvider('PrivateTutorImages');
	...
	*/
	public function actionDownload()
	{
		...
		$file_path = 'uploadedfiles/privatetutor_images/'.$model->center_id.'/'.$model->file;
	...
	*/
	...
	{
		$model = PrivateTutorImages::model()->findByAttributes(array('id'=>$_REQUEST['id']));
	...
	*/
	...
	{
		$model=new PrivateTutorImages('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['PrivateTutorImages']))
			$model->attributes=$_GET['PrivateTutorImages'];
	...
	{
		$model=PrivateTutorImages::model()->findByPk($id);
		if($model===null)
