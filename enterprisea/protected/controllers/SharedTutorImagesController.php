...
class SharedTutorImagesController extends RController
{
	/**
	...
	{
		$model = SharedTutorImages::model()->findByAttributes(array('center_id'=>$id));
	...
	{
		//echo $_POST['SharedTutorImages']['sid'];exit;
		$model=new SharedTutorImages;
		$flag = 1;
		...
		if(isset($_POST['SharedTutorImages']))
		{
			
			$list = $_POST['SharedTutorImages'];
			...
			{	$value = explode('.',$_FILES['SharedTutorImages']['name']['file'][$i]);
				$model=new SharedTutorImages;
				$model->center_id = $_POST['SharedTutorImages']['center_id'][$i];
				$model->title = $_POST['SharedTutorImages']['title'][$i];
				...
				$model->file_type = $_FILES['SharedTutorImages']['type']['file'][$i];
				
				$file_size = $_FILES['SharedTutorImages']['size']['file'][$i];
				if($model->center_id!='' and $model->title!='' and $model->file!='' and $model->file_type!='') // Checking if Document name and file is uploaded
				{
					if(in_array($model->file_type,$valid_file_types)) // Checking file type
					{
						if($file_size <= 5242880) // Checking file size
						{
							...
							if(!is_dir('uploadedfiles/sharedtutor_images/')) // Creating student_document directory
							{
								mkdir('uploadedfiles/sharedtutor_images/');
							}
							if(!is_dir('uploadedfiles/sharedtutor_images/'.$model->center_id)) // Creating student directory for saving the files
							{
								mkdir('uploadedfiles/sharedtutor_images/'.$model->center_id);
							}
							$temp_file_loc = $_FILES['SharedTutorImages']['tmp_name']['file'][$i];
							$destination_file = 'uploadedfiles/sharedtutor_images/'.$model->center_id.'/'.$model->file;
			...
			if($flag == 1) // If no errors, go to next step of the student registration
			{
				...
				if($_POST['SharedTutorImages']['document']==1)
				{
					$this->redirect(array('sharedTutorImages/create','id'=>$_POST['SharedTutorImages']['sid']));
				}
				else
				{
				
					$this->redirect(array('sharedTutorImages/view','id'=>$_POST['SharedTutorImages']['sid']));
				}
			}
			else // If errors are present, redirect to the same page
			{
				...
				{
					$this->redirect(array('create','id'=>$_POST['SharedTutorImages']['sid']));
				}
			}
		} // END isset
	...
	public function actionUpdate()
	{
		...
		if(isset($_POST['SharedTutorImages']))
		{
			...
			{
				if($_FILES['SharedTutorImages']['name']['file']!=NULL)
				{
					$value = explode('.',$_FILES['SharedTutorImages']['name']['file']);
					$extension = end($value); // Get extension of the file
					$model->file = $this->generateRandomString(rand(6,10)).'.'.$extension; // Generate random string as filename
					$model->file_type = $_FILES['SharedTutorImages']['type']['file'];
					$file_size = $_FILES['SharedTutorImages']['size']['file'];
					if(in_array($model->file_type,$valid_file_types)) // Checking file type
					{
						if($file_size <= 5242880) // Checking file size
						{
							...
							$temp_file_loc = $_FILES['SharedTutorImages']['tmp_name']['file'];
							$destination_file = 'uploadedfiles/sharedtutor_images/'.$model->center_id.'/'.$model->file;
			...
			{ 
				if($model->save())
				{
					if($_FILES['SharedTutorImages']['name']['file']!=NULL)
					{
						$old_destination_file = 'uploadedfiles/sharedtutor_images/'.$model->center_id.'/'.$old_model['file'];	
						if(file_exists($old_destination_file))
						{
							unlink($old_destination_file);
						}
					}
					$this->redirect(array('sharedTutorImages/create','id'=>$model->center_id));
				}
				else
				{
	...
	public function actionDeletes()
	{
		...
		$this->redirect(array('sharedTutorImages/create','id'=>$_REQUEST['id']));
	}
	...
	{
		$dataProvider=new CActiveDataProvider('SharedTutorImages');
	...
	*/
	public function actionDownload()
	{
		...
		$file_path = 'uploadedfiles/sharedtutor_images/'.$model->center_id.'/'.$model->file;
	...
	*/
	...
	{
		$model = SharedTutorImages::model()->findByAttributes(array('id'=>$_REQUEST['id']));
	...
	*/
	...
	{
		$model=new SharedTutorImages('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['SharedTutorImages']))
			$model->attributes=$_GET['SharedTutorImages'];
	...
	{
		$model=SharedTutorImages::model()->findByPk($id);
		if($model===null)
