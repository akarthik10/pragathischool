...
class CenterImagesController extends Controller
{
	/**
...
*/
	...
	{
		$model = CenterImages::model()->findByAttributes(array('center_id'=>$id));
	...
	{
		//echo $_POST['CenterImages']['sid'];exit;
		$model=new CenterImages;
		$flag = 1;
		...
		if(isset($_POST['CenterImages']))
		{
			
			$list = $_POST['CenterImages'];
			...
			{	$value = explode('.',$_FILES['CenterImages']['name']['file'][$i]);
				$model=new CenterImages;
				$model->center_id = $_POST['CenterImages']['center_id'][$i];
				$model->title = $_POST['CenterImages']['title'][$i];
				...
				$model->file_type = $_FILES['CenterImages']['type']['file'][$i];
				
				$file_size = $_FILES['CenterImages']['size']['file'][$i];
				if($model->center_id!='' and $model->title!='' and $model->file!='' and $model->file_type!='') // Checking if Document name and file is uploaded
				{
					if(in_array($model->file_type,$valid_file_types)) // Checking file type
					{
						if($file_size <= 5242880) // Checking file size
						{
							...
							if(!is_dir('uploadedfiles/center_images/')) // Creating student_document directory
							{
								mkdir('uploadedfiles/center_images/');
							}
							if(!is_dir('uploadedfiles/center_images/'.$model->center_id)) // Creating student directory for saving the files
							{
								mkdir('uploadedfiles/center_images/'.$model->center_id);
							}
							$temp_file_loc = $_FILES['CenterImages']['tmp_name']['file'][$i];
							$destination_file = 'uploadedfiles/center_images/'.$model->center_id.'/'.$model->file;
			...
			if($flag == 1) // If no errors, go to next step of the student registration
			{
				...
				if($_POST['CenterImages']['document']==1)
				{
					$this->redirect(array('centerImages/create','id'=>$_POST['CenterImages']['sid']));
				}
				else
				{
				
					$this->redirect(array('centerImages/view','id'=>$_POST['CenterImages']['sid']));
				}
			}
			else // If errors are present, redirect to the same page
			{
				...
				{
					$this->redirect(array('create','id'=>$_POST['CenterImages']['sid']));
				}
			}
		} // END isset
	...
	public function actionUpdate()
	{
		...
		if(isset($_POST['CenterImages']))
		{
			...
			{
				if($_FILES['CenterImages']['name']['file']!=NULL)
				{
					$value = explode('.',$_FILES['CenterImages']['name']['file']);
					$extension = end($value); // Get extension of the file
					$model->file = $this->generateRandomString(rand(6,10)).'.'.$extension; // Generate random string as filename
					$model->file_type = $_FILES['CenterImages']['type']['file'];
					$file_size = $_FILES['CenterImages']['size']['file'];
					if(in_array($model->file_type,$valid_file_types)) // Checking file type
					{
						if($file_size <= 5242880) // Checking file size
						{
							...
							$temp_file_loc = $_FILES['CenterImages']['tmp_name']['file'];
							$destination_file = 'uploadedfiles/center_images/'.$model->center_id.'/'.$model->file;
			...
			{ 
				if($model->save())
				{
					if($_FILES['CenterImages']['name']['file']!=NULL)
					{
						$old_destination_file = 'uploadedfiles/center_images/'.$model->center_id.'/'.$old_model['file'];	
						if(file_exists($old_destination_file))
						{
							unlink($old_destination_file);
						}
					}
					$this->redirect(array('centerImages/create','id'=>$model->center_id));
				}
				else
				{
	...
	public function actionDeletes()
	{
		...
		$this->redirect(array('centerImages/create','id'=>$_REQUEST['id']));
	}
	...
	{
		$dataProvider=new CActiveDataProvider('CenterImages');
	...
	*/
	public function actionDownload()
	{
		...
		$file_path = 'uploadedfiles/center_images/'.$model->center_id.'/'.$model->file;
	...
	*/
	...
	{
		$model = CenterImages::model()->findByAttributes(array('id'=>$_REQUEST['id']));
	...
	*/
	...
	{
		$model=new CenterImages('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['CenterImages']))
			$model->attributes=$_GET['CenterImages'];
	...
	{
		$model=CenterImages::model()->findByPk($id);
		if($model===null)
