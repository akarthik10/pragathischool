...
class PrivateTutorController extends Controller
{
	...
	{
		$model=new PrivateTutor;
		$model->location_lat = -34.397;
		$model->location_long = 150.644;
		$model_1 = new PrivateTutorExperience;
		$model_course= new PrivateTutorCourse;
		...
		if(!empty($_POST))
		{
			...
			$model->attributes=$_POST['PrivateTutor'];
			$model->location_lat = $_POST['PrivateTutor']['location_lat'];
			$model->location_long = $_POST['PrivateTutor']['location_long'];
			...
			if(isset($_POST['PrivateTutorCourse']))
			{	
				foreach($_POST['PrivateTutorCourse']['course_id'] as $val)
				{	
					$model_course= new PrivateTutorCourse;
			...
			else
			{
				...
				$model_1->attributes=$_POST['PrivateTutorExperience'];
			...
			if($validate)
			{
				if($model->save())
				{
					...
					$uid = PrivateTutor::model()->add_admin($tenant,$model->id);//add admin user for center
					$model->saveAttributes(array('uid'=>$uid));
					PrivateTutor::model()->add_settings($tenant);//add default settings to db
					PrivateTutor::model()->add_weekdays($tenant);//add default weekdays to db
					PrivateTutor::model()->add_grading_levels($tenant);//add default Grade levels to db
	...
	public function actionUpdate($id)
	{
		...
		$model_1 =PrivateTutorExperience::model()->findByAttributes(array('center_id'=>$model->id));
		$model_course= new PrivateTutorCourse;
		//$model_service = new ServiceCenter;
		...
		if(!empty($_POST))
		{
			...
			{	
				...
				PrivateTutorCourse::model()->deleteAllByAttributes(array('center_id'=>$model->id));
				foreach($_POST['PrivateTutorCourse']['course_id'] as $val)
				{	
					$model_course= new PrivateTutorCourse;
					$model_course->center_id = $model->id;
	...
	public function actionDelete($id)
	{
		...
		$old_courses = PrivateTutorCourse::model()->findAllByAttributes(array('center_id'=>$id));
		foreach($old_courses as $old_course)
		{
			$courses = Courses::model()->findAllByAttributes(array('category_id'=>$old_course->course_id));
			foreach($courses as $course)
		...
		PrivateTutorCourse::model()->deleteAllByAttributes(array('center_id'=>$id));
		
		//delete experience
		PrivateTutorExperience::model()->deleteAllByAttributes(array('center_id'=>$id));
		//delete location details
		PrivateTutorLocation::model()->deleteAllByAttributes(array('center_id'=>$id));
		
		//delete service details
		//ServiceCenter::model()->deleteAllByAttributes(array('center_id'=>$id));
		
		//delete uploaded documents
		$documents = PrivateTutorUploads::model()->findAllByAttributes(array('center_id'=>$id));
		foreach($documents as $document)
		{
			
			$destination_file = 'uploadedfiles/privatetutor_document/'.$document->center_id.'/'.$document->file;
		...
		//delete tutor
		$center_user = PrivateTutor::model()->findByPk($id);
		User::model()->deleteAllByAttributes(array('id'=>$center_user->uid));
		PrivateTutor::model()->deleteAllByAttributes(array('id'=>$id));
	...
	public function actionIndex()
	{
		...
		$centers= PrivateTutor::model()->findAllByAttributes(array('user_type'=>2));
	...
	{
		$model=new PrivateTutor('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['PrivateTutor']))
	...
	{
		$model=PrivateTutor::model()->findByPk($id);
		if($model===null)
