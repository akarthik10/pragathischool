...
class CenterUploadsController extends Controller
{
	/**
...
*/
	...
	{
		$model = CenterUploads::model()->findByAttributes(array('center_id'=>$id));
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['CenterUploads']))
		{
			
			$list = $_POST['CenterUploads'];
			...
			{	$value = explode('.',$_FILES['CenterUploads']['name']['file'][$i]);
				$model=new CenterUploads;
				$model->center_id = $_POST['CenterUploads']['center_id'][$i];
				$model->title = $_POST['CenterUploads']['title'][$i];
				...
				$model->file_type = $_FILES['CenterUploads']['type']['file'][$i];
				
				$file_size = $_FILES['CenterUploads']['size']['file'][$i];
				if($model->center_id!='' and $model->title!='' and $model->file!='' and $model->file_type!='') // Checking if Document name and file is uploaded
				{
					if(in_array($model->file_type,$valid_file_types)) // Checking file type
					{
						if($file_size <= 5242880) // Checking file size
						{
							...
							if(!is_dir('uploadedfiles/center_document/')) // Creating student_document directory
							{
								mkdir('uploadedfiles/center_document/');
							}
							if(!is_dir('uploadedfiles/center_document/'.$model->center_id)) // Creating student directory for saving the files
							{
								mkdir('uploadedfiles/center_document/'.$model->center_id);
							}
							$temp_file_loc = $_FILES['CenterUploads']['tmp_name']['file'][$i];
							$destination_file = 'uploadedfiles/center_document/'.$model->center_id.'/'.$model->file;
			...
			if($flag == 1) // If no errors, go to next step of the student registration
			{
				...
				if($_POST['CenterUploads']['document']==1)
				{
					$this->redirect(array('centerUploads/create','id'=>$_POST['CenterUploads']['sid']));
				}
				else
				{
				
					$this->redirect(array('centerUploads/view','id'=>$_POST['CenterUploads']['sid']));
				}
			}
			else // If errors are present, redirect to the same page
			{
				...
				{
					$this->redirect(array('create','id'=>$_POST['CenterUploads']['sid']));
				}
			}
		} // END isset
	...
	public function actionUpdate()
	{
		...
		if(isset($_POST['CenterUploads']))
		{
			...
			{
				if($_FILES['CenterUploads']['name']['file']!=NULL)
				{
					$value = explode('.',$_FILES['CenterUploads']['name']['file']);
					$extension = end($value); // Get extension of the file
					$model->file = $this->generateRandomString(rand(6,10)).'.'.$extension; // Generate random string as filename
					$model->file_type = $_FILES['CenterUploads']['type']['file'];
					$file_size = $_FILES['CenterUploads']['size']['file'];
					if(in_array($model->file_type,$valid_file_types)) // Checking file type
					{
						if($file_size <= 5242880) // Checking file size
						{
							...
							$temp_file_loc = $_FILES['CenterUploads']['tmp_name']['file'];
							$destination_file = 'uploadedfiles/center_document/'.$model->center_id.'/'.$model->file;
			...
			{ 
				if($model->save())
				{
					if($_FILES['CenterUploads']['name']['file']!=NULL)
					{
						$old_destination_file = 'uploadedfiles/center_document/'.$model->center_id.'/'.$old_model['file'];	
						if(file_exists($old_destination_file))
						{
							unlink($old_destination_file);
						}
					}
					$this->redirect(array('centerUploads/create','id'=>$model->center_id));
				}
				else
				{
	...
	public function actionDeletes()
	{
		...
		$this->redirect(array('centerUploads/create','id'=>$_REQUEST['id']));
	}
	...
	{
		$dataProvider=new CActiveDataProvider('CenterUploads');
	...
	*/
	public function actionDownload()
	{
		...
		$file_path = 'uploadedfiles/center_document/'.$model->center_id.'/'.$model->file;
	...
	*/
	...
	{
		$model = CenterUploads::model()->findByAttributes(array('id'=>$_REQUEST['id']));
	...
	*/
	...
	{
		$model=new CenterUploads('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['CenterUploads']))
			$model->attributes=$_GET['CenterUploads'];
	...
	{
		$model=CenterUploads::model()->findByPk($id);
		if($model===null)
